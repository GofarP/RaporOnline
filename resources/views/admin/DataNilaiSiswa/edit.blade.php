    ...
    @section('content')
        <div class="card shadow mb-4 w-100">
            ...
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('admin_update_data_nilai_siswa',$data_nilai->id_nilai_master)}}">
                    @csrf
                    @method("PUT")

                    <div class="mb-3">
                        <label for="email" class="mt-3 mb-2">NISN:</label>
                        <label for="name" class="mt-3 mb-2">{{ $data_nilai->NISN }}</label>
                        <input type="hidden" value="{{ $data_nilai->NISN }}" name="nisn" />
                        <input type="hidden" value="{{$data_nilai->id_nilai_master}}" name="id_nilai_master"/>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control @error('id_mapel') is-invalid @enderror" name="id_mapel"
                            ...
                            @foreach ($data_mapel as $mapel)
                                <option value="{{ $mapel->id_mapel }}" {{$data_nilai->id_mapel==$mapel->id_mapel ? 'selected': ''}}> {{ $mapel->nama }}</option>
                            @endforeach
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="nilai" id="nilai" value="{{$data_nilai->nilai}}">
                    ...
                    <div class="mb-3">
                        <input type="submit" class="form-control btn btn-success mt-3" value="Update Nilai Siswa"
