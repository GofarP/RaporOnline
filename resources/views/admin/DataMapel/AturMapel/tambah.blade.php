    ...
    @section('content')
        <div class="card shadow mb-4 w-100">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary"> Atur Mata Peljaran</h6>
            </div>

            <div class="card-body">

                <form method="POST" action="{{ route('store_atur_mata_pelajaran') }}">
                    @csrf

                    <div class="mb-3">
                        <label for="id_mapel" class="mt-3 mb-2">ID Mata Pelajaran:</label>
                        <label class="mt-3 mb-2">{{ $mapel->id_mapel }}</label>
                        <input type="hidden" name="id_mata_pelajaran" value="{{ $mapel->id_mapel }}">
                    </div>

                    <div class="mb-3">
                        <label class="mt-3 mb-2">Nama Mata Pelajaran:</label>
                        <label class="mt-3 mb-2">{{ $mapel->nama }}</label>
                    </div>
                    ...
                    <div class="mb-3">
                        <input type="submit" class="form-control btn btn-success mt-3" value="Atur Mata Pelajaran"
